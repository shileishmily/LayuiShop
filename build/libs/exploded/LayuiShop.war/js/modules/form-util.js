...
layui.define('form',function(exports) {
	...
	var formUtil = {
		...
		/**
		 * 动态渲染下拉框
		 * @param targetSelector select元素ID
		 * @param templates 下拉框数据
		 * @param isAll 是否显示全部option
		 * @param isDisable 是否Disable
		 */
		renderSelects: function (targetSelector, templates, isAll, isDisable) {
			$(targetSelector).html("");
		 ...
		 */
		renderCheckboxes: function(targetSelector, datas, targetName,layFilter,isDisable,verify) {
			$(targetSelector).html("");
			var checkboxes = '';
			var filter = layFilter?'lay-filter="'+layFilter+'"':'';
			var disable = isDisable?'disabled':'';
			var layVerify = verify?'lay-verify="'+verify+'"':'';

			$.each(datas, function(index, item) {
				checkboxes += '<input type="checkbox" '+disable+' ' + layVerify +' '+filter+' name="' + targetName + '" value="' + item.key + '" title="'+item.value+'"/>';
			});
		 ...
		 */
		renderRadios: function(targetSelector, datas,targetName,layFilter,isDisable,verify) {
			...
			var layVerify = verify?'lay-verify="'+verify+'"':'';
			$.each(datas, function(index, item) {
				radios += '<input type="radio" name="' + targetName + '" ' +disable+' ' + layVerify +' '+ filter + ' value="' + item.key + '" title="'+item.value+'"/>';
			});
